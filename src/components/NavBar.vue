  ...
  <nav class="sticky top-0 z-50 w-full transition-all duration-300 border-b backdrop-blur-xl border-zinc-200 dark:border-zinc-800 bg-nav-glass">
    <div class="container px-4 mx-auto h-20 flex items-center justify-between gap-6">
      ...
      <RouterLink to="/" class="flex items-center gap-3 group relative overflow-hidden rounded-xl p-1">
        ...
        <span class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-zinc-800 to-zinc-600 dark:from-white dark:to-zinc-300 hidden sm:block tracking-tight">Aarz Store</span>
      </RouterLink>
      ...
      <div class="flex items-center gap-3">
        ...
        <RouterLink 
          to="/wishlist" 
          class="relative p-2 rounded-xl text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all hover:text-pink-500 dark:hover:text-pink-400 group"
          title="View Wishlist"
        >
           <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
           
           <!-- Wishlist Badge -->
           <span v-if="store.wishlist && store.wishlist.length > 0" class="absolute -top-1 -right-1 bg-pink-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm animate-in zoom-in">
        ...
        <button 
          @click="toggleCart" 
          class="relative p-2 rounded-xl text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all hover:text-violet-600 dark:hover:text-violet-400 group"
          title="View Cart"
        >
           ...
           <!-- Badge -->
           <span v-if="store.cartCount > 0" class="absolute -top-1 -right-1 bg-violet-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm animate-in zoom-in">
             {{ store.cartCount }}
           </span>
        </button>
